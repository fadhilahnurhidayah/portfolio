...
function About() {
    ...
    const skills = [
        ...
        { icon: Clock, title: 'Time Mgmt' },
    ];

    const stats = [
        { value: '3+', label: 'Projects' },
        { value: '5+', label: 'Events' },
        { value: '100+', label: 'Bantuan' },
    ...
    return (
        ...
        >
            <div className="w-full flex flex-col items-center gap-12">
                ...
                {/* Bio */}
                <div
                    className={`w-full max-w-5xl p-12 sm:p-16 rounded-[3rem] text-center hover-lift ${theme === 'light'
                        ? 'bg-white shadow-xl shadow-gray-100/50'
                        : 'bg-gray-800/50 border border-gray-700 backdrop-blur-sm'
                        }`}
                    style={{ marginBottom: '3rem' }}
                >
                    <p className={`text-2xl leading-relaxed mb-10 ${theme === 'light' ? 'text-gray-700' : 'text-gray-300'
                        }`}>
                ...
                {/* Stats */}
                <div
                    className="w-full max-w-6xl grid grid-cols-2 md:grid-cols-4 gap-12 mb-32"
                >
                    {stats.map((stat, idx) => (
                        <motion.div
                            key={stat.label}
                            className={`p-10 rounded-[2.5rem] text-center hover-lift ${theme === 'light'
                                ? 'bg-white shadow-xl shadow-gray-100/50'
                            ...
                            viewport={{ once: true }}
                        >
                            <div className="text-5xl sm:text-6xl font-bold bg-linear-to-r from-[#7d1f2f] to-[#d4a574] bg-clip-text text-transparent mb-4">
                                {stat.value}
                            </div>
                ...
                {/* Skills */}
                <h3 className={`text-3xl font-bold mb-20 text-center ${theme === 'light' ? 'text-gray-900' : 'text-white'
                    }`}>
                    Kemampuan
                </h3>
                <div className="w-full max-w-6xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
                    {skills.map((skill, idx) => (
                        <motion.div
                            key={skill.title}
                            className={`flex items-center gap-8 p-8 sm:p-10 rounded-[2.5rem] hover-lift ${theme === 'light'
                                ? 'bg-white shadow-xl shadow-gray-100/50'
                                : 'bg-gray-800 border border-gray-700'
                                }`}
                            initial={{ opacity: 0, y: 20 }}
                            ...
                            viewport={{ once: true }}
                        >
                            <div className={`w-20 h-20 rounded-3xl flex flex-shrink-0 items-center justify-center ${theme === 'light' ? 'bg-[#7d1f2f]/10' : 'bg-[#7d1f2f]/30'
                                }`}>
                                <skill.icon className="w-10 h-10 text-[#7d1f2f]" />
                            </div>
